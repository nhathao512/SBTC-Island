...
export default function FinalCTA() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      // Silhouette fade in from below on scroll
  ...
  return (
    ...
    >
      {/* T-Rex silhouette (pure CSS geometric shape) */}
      <div
        ref={silRef}
        aria-hidden="true"
        style={{
      ...
      >
        ...
        >
          ...
          {[0, 1, 2, 3, 4].map((i) => (
            <polygon
              key={i}
              points={`${185 + i * 10},${175 - i * 2} ${190 + i * 10},${155 - i * 2} ${195 + i * 10},${175 - i * 2}`}
            />
          ))}
        </svg>
      </div>
      ...
      <div
        style={{
          position: 'absolute',
          inset: 0,
          background: 'radial-gradient(ellipse 70% 60% at 50% 80%, rgba(200,116,42,0.05) 0%, transparent 70%)',
          pointerEvents: 'none',
          zIndex: 2,
        }}
      />
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: '54rem' }}>
        <div className="text-label" style={{ marginBottom: '2rem' }}>THE WORLD AWAITS</div>
        ...
        <div ref={textRef}>
          ...
          >
            <span style={{ display: 'block', color: '#F5F5F5' }}>READY</span>
            <span style={{ display: 'block', color: 'var(--color-accent)' }}>TO ENTER</span>
            <span style={{ display: 'block', color: '#F5F5F5' }}>{siteConfig.shortName}?</span>
          </h2>
        </div>
        ...
        >
          <div className="status-badge" style={{ justifyContent: 'center' }}>
            ...
            <span className="text-label" style={{ color: 'var(--color-online)' }}>
              {serverConfig.status}
            </span>
          </div>
          <div
          ...
          >
            {siteConfig.region} REGION — {serverConfig.ip}
          </div>
          <div className="text-label" style={{ color: '#2A2A2A', marginTop: '2.5rem' }}>
            {siteConfig.name}
          </div>
        </div>
      </div>
